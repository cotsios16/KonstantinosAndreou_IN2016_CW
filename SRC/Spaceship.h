...
public:
	...
	// Invulnerability (Part II - Invulnerability Power-up)
	void SetInvulnerable(int duration_ms) { mInvulnerableTimer = duration_ms; }
	bool IsInvulnerable() const           { return mInvulnerableTimer > 0; }

private:
	float mThrust;
	int   mInvulnerableTimer; // counts down in ms; >0 means ship is invulnerable
